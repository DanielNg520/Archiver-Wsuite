...
| `routing.py` | canonicalize `[<label>~]chat_id[.t<topic>]` token (dash-free→`-100…`, cosmetic label stripped) | `parse_route`, `Route`, `is_chat_id` |
| `grouping.py` | split-part album group keys | `split_group_key`, `is_split_group` |
| `files.py` | media-type sets (`PHOTO_EXTS`/`INLINE_VIDEO_EXTS`/`DOCUMENT_VIDEO_EXTS`/`VIDEO_EXTS`/`MEDIA_EXTENSIONS`, `ALBUM_MAX`) + album buckets: `media_bucket` (photo/video/single, all producers) and `orphaned_kind` (`media` = photos+inline video, `document` = non-inline containers `.mkv`/`.wmv`/`.avi`/…, `.gif`, other — chat_id folders), unified by source-aware `album_bucket`; sidecar-aware delete; `prune_empty_dirs` (empty-folder auto-cleaner, spares `#`-prefixed hashtag buckets). Leaf module — also THE home of `ORPHANED_SOURCE_NAME` (re-exported by `orphaned.py` as `ORPHANED_SOURCE`) | `media_bucket`, `orphaned_kind`, `album_bucket`, `cleanup_sidecars`, `prune_empty_dirs` |
