...
| **Root** | the checkout (e.g. `~/Coding/Archiver-Wsuite/Linux`) — config + DB self-contained under `<repo>/.config` |
...
| `ops` | install/health/logs via systemd; reads artifacts + DB read-only, never a worker. |
